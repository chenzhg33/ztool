...
export default {
  ...
  methods: {
    ...
    onlyShowDiff() {
      ...
      var originHideRanges = [];
      var modifyHideRanges = [];
      ...
      lineChanges.push({
        originalStartLineNumber: versionContentLines + 1,
        originalEndLineNumber: versionContentLines + 1,
        modifiedStartLineNumber: onlineContentLines + 1,
        modifiedEndLineNumber: onlineContentLines + 1,
      });
      console.log("calc lineChanges");
      console.log(lineChanges);
      for (var i = 0; i < lineChanges.length - 1; i++) {
        if (
          lineChanges[i].originalEndLineNumber + 1 <=
          lineChanges[i + 1].originalStartLineNumber - 1
        ) {
          originHideRanges.push(
            new monaco.Range(
              lineChanges[i].originalEndLineNumber + 1,
              1,
              lineChanges[i + 1].originalStartLineNumber - 1,
        ...
        if (
          lineChanges[i].modifiedEndLineNumber + 1 <=
          lineChanges[i + 1].modifiedStartLineNumber - 1
        ) {
          modifyHideRanges.push(
            new monaco.Range(
              lineChanges[i].modifiedEndLineNumber + 1,
              1,
              lineChanges[i + 1].modifiedStartLineNumber - 1,
